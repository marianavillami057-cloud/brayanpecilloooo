...
router.post("/upload-signature", requireAuth, async (req, res) => {
  try {
    ...
    const apiKey = keyRow?.value ?? process.env.CLOUDINARY_API_KEY;
    const apiSecret = secretRow?.value ?? process.env.CLOUDINARY_API_SECRET;
    const cloudName = nameRow?.value ?? process.env.CLOUDINARY_CLOUD_NAME;
